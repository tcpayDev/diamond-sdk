...
public class AnnotationUtil {
  ...
  public static final HttpUtil.Request buildReq(String url, Object req, String secret)
      throws Exception {
    return buildReq(url, req, secret,new TreeMap());
  }

  public static final HttpUtil.Request buildReq(String url, Object req, String secret, TreeMap treeMap)
      throws Exception {
    ...
    for (Field field : fields) {
      ...
      if (null != field.get(req)) {
        data.put(field.getName(),
            field.get(req) instanceof String ? field.get(req) : JSON.toJSONString(field.get(req)));
      }
    }
    return new HttpUtil.Request(url, data, secret,treeMap);
  }
}
